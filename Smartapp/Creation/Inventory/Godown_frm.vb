...
Public Class Godown_frm
    ...
    Dim close_focus_obj As TXT
    Private Sub Godown_frm_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        BG_frm.HADE_TXT.Text = "Stock Godown"
    ...
    Private Sub B_5_Click(sender As Object, e As EventArgs)
        If BG_frm.From_ID = From_ID Then
            Cell("Audit Analysis", "", "Stock Godown", VC_ID_)
        End If
    End Sub
    Private Function Insurt_Audit() As Boolean
        ...
        If Audit_Save("TBL_Stock_Godown", "ID", VC_ID_) = True Then
            Return True
        End If
    End Function
    Private Function Delete_Data()
        ...
        If Chack_Delete_Allow() = True Then
            If Msg_Yn("Are you soure", "You really want to delete the Godown") = DialogResult.Yes Then
                If Insurt_Audit() = True Then
                    qury = $"DELETE FROM TBL_Stock_Godown Where ID = '{VC_ID_}';"
    ...
    Private Function Chack_Delete_Allow() As Boolean
        If Chack_Duplicate(Database_File.cre, "TBL_VC", "ID", "Godown = '" & VC_ID_ & "' and Visible = 'Approval'") = True Then
            Return False
        End If
        Return True
    End Function
    Private Sub All_Enter(sender As Object, e As EventArgs) Handles Name_TXT.Enter, Alias_TXT.Enter
        Enter_(sender)
    End Sub
    Private Sub All_Lostfocus(sender As Object, e As EventArgs) Handles Name_TXT.LostFocus, Alias_TXT.LostFocus
        Lost_(sender)
        List_frm.Dispose()
        NOT_Clear()
    End Sub
    ...
    Private Sub Godown_frm_Enter(sender As Object, e As EventArgs) Handles Me.Enter
    ...
    Private Function Save_Requr() As Boolean
        ...
        If Name_TXT.Text = Alias_TXT.Text Then
            ...
            Exit Function
        End If
        If Chack_Duplicate(Database_File.cre, "TBL_Stock_Godown", "Name", Duplicate_Type) = True Then
            Msg(NOT_Type.Erro, "Duplicate - Name", Text_Action(vl.Already_Exists, "Name"))
            Name_TXT.Focus()
            Return False
            Exit Function
        End If
        If Chack_Duplicate(Database_File.cre, "TBL_Stock_Godown", "Alias", Duplicate_Type1) = True And Alias_TXT.Text <> "" Then
            Msg(NOT_Type.Erro, "Duplicate - Name", Text_Action(vl.Already_Exists, "Name"))
            Return False
            Exit Function
        End If

        If Chack_Duplicate(Database_File.cre, "TBL_Stock_Godown", "Name", Duplicate_Alias_Type) = True Then
            Msg(NOT_Type.Erro, "Duplicate - Alias", Text_Action(vl.Already_Exists, "Alias"))
            Name_TXT.Focus()
            Return False
            Exit Function
        End If
        If Chack_Duplicate(Database_File.cre, "TBL_Stock_Godown", "Alias", Duplicate_Alias_Type1) = True And Alias_TXT.Text <> "" Then
            Msg(NOT_Type.Erro, "Duplicate - Alias", Text_Action(vl.Already_Exists, "Alias"))
            Return False
            Exit Function
        End If
        Return True
    End Function
    Private Sub Godown_frm_KeyDown(sender As Object, e As KeyEventArgs) Handles Me.KeyDown
    ...
    Private Function Insurt_Value() As Boolean
        If VC_Type_ = "Create" Or VC_Type_ = "Create_Close" Then
            If open_MSSQL(Database_File.cre) Then
                qury = "INSERT INTO TBL_Stock_Godown (Name,Nickname,Company,Visible,[User],Date_install,PC) VALUES (@Name, @Nickname,@Company,@Visible,@User,@Install_,@PC)"
        ...
        Else
            ...
            If open_MSSQL(Database_File.cre) Then
                qury = "UPDATE TBL_Stock_Godown SET Name = @Name,Nickname = @Nickname,[User] = @User,Date_install = @Install_,PC = @PC WHERE ID = '" & VC_ID_ & "'"
                cmd = New SQLiteCommand(qury, con)
                Try
                    With cmd.Parameters
                        .AddWithValue("@Name", Name_TXT.Text.Trim)
    ...
    Private Function Clear_all()
        ...
        End If
    End Function
    Private Sub Godown_frm_Disposed(sender As Object, e As EventArgs) Handles Me.Disposed
        BG_frm.BG_Path_TXT.Text = Path_End
    ...
    Private Function Display_Fill_All()
        Name_TXT.Text = Find_DT_Value(Database_File.cre, "TBL_Stock_Godown", "Name", "ID = '" & VC_ID_ & "'")
        Alias_TXT.Text = Find_DT_Value(Database_File.cre, "TBL_Stock_Godown", "Nickname", "ID = '" & VC_ID_ & "'")
    End Function
    Private Function Name_aLlow() As Boolean
        ...
        If Chack_Duplicate(Database_File.cre, "TBL_Stock_Godown", "Name", Duplicate_Type) = True Then
            ...
            Return False
            Exit Function
        ElseIf Chack_Duplicate(Database_File.cre, "TBL_Stock_Godown", "Nickname", Duplicate_Type1) = True Then
            NOT_("The Name Entered is a 'Duplicate'", NOT_Type.Warning)
            Return False
            Exit Function
        Else
    ...
    Private Function Alisa_aLlow() As Boolean
        ...
        If Alias_TXT.Text <> "" Then
            ...
            If Chack_Duplicate(Database_File.cre, "TBL_Stock_Godown", "Nickname", Duplicate_Alias_Type) = True Then
                NOT_("The Alias Entered is a 'Duplicate'", NOT_Type.Warning)
                Return False
                Exit Function
            ElseIf Chack_Duplicate(Database_File.cre, "TBL_Stock_Godown", "Name", Duplicate_Alias_Type1) = True Then
                NOT_("The Alias Entered is a 'Duplicate'", NOT_Type.Warning)
                Return False
                Exit Function
            Else
    ...
    Private Sub Alias_TXT_KeyDown(sender As Object, e As KeyEventArgs) Handles Alias_TXT.KeyDown
    ...
    Private Sub Godown_frm_GotFocus(sender As Object, e As EventArgs) Handles Me.GotFocus
        BG_frm.From_ID = From_ID
    End Sub
End Class