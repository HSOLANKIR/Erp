...
Public Class Stock_Group_frm
    ...
    Private Sub Inventory_Group_frm_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        '[MsgBox(VC_ID_ & " - " & Stock_Pay)

        If Stock_Pay = "Stock Group" Then
            BG_frm.HADE_TXT.Text = "Stock Group"
        ElseIf Stock_Pay = "Payroll Group" Then
            BG_frm.HADE_TXT.Text = "Payroll Group"
        End If
        Set_msg()
    ...
    Private Function Insurt_Audit() As Boolean
        ...
        If Audit_Save("TBL_Stock_Group", "ID", VC_ID_) = True Then
            Return True
        End If
    End Function
    ...
    Private Sub B_5_Click(sender As Object, e As EventArgs)
        If BG_frm.From_ID = From_ID Then
            Cell("Audit Analysis", "", "Stock Group", VC_ID_)
        End If
    End Sub
    Private Function Delete_Data()
        ...
        If Chack_Delete_Allow() = True Then
            If Msg_Delete_YN(Name_TXT, "Stock Group") = DialogResult.Yes Then
                If Insurt_Audit() = True Then
                    qury = $"DELETE FROM TBL_Stock_Group Where ID = '{VC_ID_}';"
            ...
            End If
        Else
            Msg(NOT_Type.Info, "You cannot delete", "You cannot delete this group because the group you selected is made up of vouchers")
        End If
    End Function
    Private Sub Inventory_Group_frm_Enter(sender As Object, e As EventArgs) Handles Me.Enter
        BG_frm.TYP_TXT.Text = VC_Type_
    ...
    Private Function Save_Requr() As Boolean
        ...
        If Name_TXT.Text = Alias_TXT.Text Then
            ...
            Exit Function
        End If
        If Chack_Duplicate(Database_File.cre, "TBL_Stock_Group", "Name", Duplicate_Type) = True Then
            Msg(NOT_Type.Erro, "Duplicate - Name", Text_Action(vl.Already_Exists, "Name"))
            Name_TXT.Focus()
            Return False
            Exit Function
        End If
        If Chack_Duplicate(Database_File.cre, "TBL_Stock_Group", "Nickname", Duplicate_Type1) = True Then
            Msg(NOT_Type.Erro, "Duplicate - Name", Text_Action(vl.Already_Exists, "Name"))
            Return False
            Exit Function
        End If
        ''
        If Chack_Duplicate(Database_File.cre, "TBL_Stock_Group", "Nickname", Duplicate_Alias_Type) = True Then
            Msg(NOT_Type.Erro, "Duplicate - Alias", Text_Action(vl.Already_Exists, "Alias"))
            Return False
            Exit Function
        End If

        If Chack_Duplicate(Database_File.cre, "TBL_Stock_Group", "Nickname", Duplicate_Alias_Type1) = True And Alias_TXT.Text <> "" Then
            Msg(NOT_Type.Erro, "Duplicate - Alias", Text_Action(vl.Already_Exists, "Alias"))
            Return False
            Exit Function
        End If
...
Select count(*), 'Defolt Stock Group' as Type From TBL_Stock_Group sg WHERE
sg.id = '{VC_ID_}' and
sg.Company = 'All'", cn)
    ...
    Private Function Insurt_Value() As Boolean
        If VC_Type_ = "Create" Or VC_Type_ = "Create_Close" Then
            If open_MSSQL(Database_File.cre) Then
                qury = "INSERT INTO TBL_Stock_Group (Name,Nickname,Head,Company,[User],Date_install,Visible,PC) VALUES (@Name, @Nickname,@Head,@Company,@User,@Install_,@Visible,@PC)"
                ...
                Try
                    With cmd.Parameters
                        ...
                        ElseIf BG_frm.HADE_TXT.Text = "Payroll Group" Then
        ...
        Else
            If open_MSSQL(Database_File.cre) Then
                qury = "UPDATE TBL_Stock_Group SET Name = @Name,Nickname = @Nickname,[User] = @User,Date_install = @Install_,PC = @PC WHERE ID  = '" & VC_ID_ & "'"
                cmd = New SQLiteCommand(qury, con)
                Try
                    With cmd.Parameters
                        .AddWithValue("@Name", Name_TXT.Text.Trim)
    ...
    Private Function Clear_all()
        Dim cn As New SQLiteConnection
        If open_MSSQL_Cstm(Database_File.cre, cn) = True Then
            'BG_frm.Stock_Group_BS.DataSource = Nothing
            'BG_frm.Stock_Group_BS.DataSource = Fill_Stock_Group(cn)

            'BG_frm.Payroll_Group_BS.DataSource = Nothing
            'BG_frm.Payroll_Group_BS.DataSource = Fill_Payroll_Group(cn)
        End If

        If VC_Type_ = "Create" Then
            VC_ID_ = Find_DT_Value(Database_File.cre, "TBL_Stock_Group", "ID", $"Name = '{Name_TXT.Text}'")
    ...
    Private Function Display_Fill_All()
        Name_TXT.Text = Find_DT_Value(Database_File.cre, "TBL_Stock_Group", "Name", "ID  = '" & VC_ID_ & "'")
        Alias_TXT.Text = Find_DT_Value(Database_File.cre, "TBL_Stock_Group", "Nickname", "ID  = '" & VC_ID_ & "'")
    End Function
    Private Sub Inventory_Group_frm_KeyDown(sender As Object, e As KeyEventArgs) Handles Me.KeyDown
        If e.KeyCode = Keys.Escape Then
            If Msg_Exit_YN("Stock Group " & VC_Type_) = DialogResult.Yes Then
                Me.Dispose()
    ...
    Private Sub Inventory_Group_frm_Disposed(sender As Object, e As EventArgs) Handles Me.Disposed
    ...
    Private Function Name_aLlow() As Boolean
        ...
        If BG_frm.From_ID = From_ID Then
            If BG_frm.From_ID = From_ID Then
                Head_Type = " and (Head  = 'Stock' or Head  = 'All') and " & Company_Visible_Filter()
            Else
                Head_Type = " and (Head  = 'Payroll' or Head  = 'All') and " & Company_Visible_Filter()
            End If

            If VC_Type_ = "Create" Or VC_Type_ = "Create_Close" Then
                Duplicate_Type = " Name  = '" & Name_TXT.Text & "' COLLATE NOCASE  and " & Company_Visible_Filter() & Head_Type
            Else
                Duplicate_Type = " (Name  = '" & Name_TXT.Text & "' COLLATE NOCASE  and ID <> '" & VC_ID_ & "') and " & Company_Visible_Filter() & Head_Type
            End If
            If VC_Type_ = "Create" Or VC_Type_ = "Create_Close" Then
                Duplicate_Type1 = " Nickname  = '" & Name_TXT.Text & "' COLLATE NOCASE  and " & Company_Visible_Filter() & Head_Type
            Else
                Duplicate_Type1 = " Nickname  = '" & Name_TXT.Text & "' COLLATE NOCASE  and ID <> '" & VC_ID_ & "' and " & Company_Visible_Filter() & Head_Type
            End If
            ...
            If Chack_Duplicate(Database_File.cre, "TBL_Stock_Group", "Name", Duplicate_Type) = True Then
                ...
                Msg_Duplicat(Name_TXT, "Stock Group")
                Return False
                Exit Function
            ElseIf Chack_Duplicate(Database_File.cre, "TBL_Stock_Group", "Nickname", Duplicate_Type1) = True Then
                NOT_("The Name Entered is a 'Duplicate'", NOT_Type.Warning)
                Msg_Duplicat(Name_TXT, "Stock Group")
                Return False
                Exit Function
    ...
    Private Function Alias_aLlow() As Boolean
        If BG_frm.From_ID = From_ID Then
            If BG_frm.From_ID = From_ID Then
                Head_Type = " and (Head  = 'Stock' or Head  = 'All')"
            Else
                Head_Type = " and (Head  = 'Payroll' or Head  = 'All')"
            End If

            If VC_Type_ = "Create" Or VC_Type_ = "Create_Close" Then
                Duplicate_Alias_Type = " Name  = '" & Alias_TXT.Text & "' COLLATE NOCASE " & Head_Type
            Else
                Duplicate_Alias_Type = " Name  = '" & Alias_TXT.Text & "' COLLATE NOCASE  and ID <> '" & VC_ID_ & "'" & Head_Type
            End If

            If VC_Type_ = "Create" Or VC_Type_ = "Create_Close" Then
                Duplicate_Alias_Type1 = " Nickname  = '" & Alias_TXT.Text & "' COLLATE NOCASE " & Head_Type
            Else
                Duplicate_Alias_Type1 = " Nickname  = '" & Alias_TXT.Text & "' COLLATE NOCASE  and ID <> '" & VC_ID_ & "'" & Head_Type
            End If

            If Alias_TXT.Text <> "" Then
                If Name_TXT.Text = Alias_TXT.Text Then
                    NOT_("Name and Alias is Sam", NOT_Type.Warning)
                    Msg_Duplicat(Alias_TXT, "Stock Group")
                    Return False
                    Exit Function
                End If
                ...
                If Chack_Duplicate(Database_File.cre, "TBL_Stock_Group", "Nickname", Duplicate_Alias_Type) = True Then
                    ...
                    Return False
                    Exit Function
                ElseIf Chack_Duplicate(Database_File.cre, "TBL_Stock_Group", "Name", Duplicate_Alias_Type1) = True Then
                    NOT_("The Alias Entered is a 'Duplicate'", NOT_Type.Warning)
                    Msg_Duplicat(Alias_TXT, "Stock Group")
                    Return False
                    Exit Function
    ...
    Private Sub end_TXT_TextChanged(sender As Object, e As EventArgs) Handles end_TXT.TextChanged

    End Sub

    Private Sub end_TXT_Enter(sender As Object, e As EventArgs) Handles end_TXT.Enter
        Dim res As DialogResult = Msg_Save_YN(Name_TXT, "Stock Group")
    ...
    Private Sub Stock_Group_frm_GotFocus(sender As Object, e As EventArgs) Handles Me.GotFocus
        BG_frm.From_ID = From_ID
    End Sub
End Class